...
# MAGIC %md
# MAGIC no display until an action is specified
...
# MAGIC %md
# MAGIC Add ingestion timestamps and input_file_name() in case of an issue during the ingestion
...
# MAGIC %md
# MAGIC * Join order data to customer name
# MAGIC * format timestamp
# MAGIC * filter on quantity>0
...
# MAGIC %md
# MAGIC > We have the option `trigger(availableNow=True)` that terminate once it process all data (in micro-batch)
# MAGIC So we can combine streaming and batch processing in the same workload
# MAGIC
# MAGIC >Thanks to the `.outpumode("complete")`, we can recreate all the aggregated gold table
# MAGIC
# MAGIC >> Note :Keep in mind that Structured Streaming assumes data is only being appended in the upstream tables.
# MAGIC >> Once a table is updated or overwritten, it is no longer valid for streaming.
# MAGIC >> So in our case here, ***we cannot read a stream from this gold table.***

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM daily_customer_books LIMIT 10
...
# MAGIC %md
# MAGIC **Note that the gold layer is not updated since it's a** `.trigger(availableNow=True)`
